  ...
  return <header className="fixed top-0 w-full bg-white bg-opacity-95 backdrop-blur-md shadow-lg z-50 transition-all duration-300">
      <div className="container mx-auto px-4 py-4 bg-slate-950">
        <div className="flex items-center justify-between">
          ...
          <Button variant="ghost" size="icon" className="relative transition-all duration-300 hover:scale-110 hover:bg-emerald-50 group" onClick={onCartClick}>
            <ShoppingCart className="h-5 w-5 transition-transform duration-300 group-hover:scale-110" />
            {getTotalItems() > 0 && <span className="absolute -top-1 -right-1 bg-emerald-600 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center animate-pulse">
                {getTotalItems()}
              </span>}
          </Button>
        </div>
